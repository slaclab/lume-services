...
from lume_services.data import file
...
class FilesystemNotConfigured(Exception):
    ...
    def __init__(self, filesystem_identifier: str, configured_filesystems: List[str]):
        ...
        self.message = f"Filesystem {filesystem_identifier} not configured. Available filesystems are: {','.join(configured_filesystems)}"
...
class FileService:
    ...
    def __init__(self, filesystems: List[Filesystem]):
        ...
        """
        self._filesystems = {filesystem.identifier: filesystem for filesystem in filesystems}

    def dir_exists(self, filesystem_identifier: str, dir:str, create_dir: bool = True) -> bool:
    ...
    def read(self, filesystem_identifier: str, file: str, serializer: SerializerBase) -> Any:
    ...
    def write(self, filesystem_identifier: str, filepath: str, object: Any, serializer: SerializerBase, create_dir: bool=True):
    ...
    def _get_filesystem(self, filesystem_identifier: str) -> Filesystem:
        ...
        if not filesystem:
            logger.error()
            raise FilesystemNotConfigured(filesystem_identifier, list(self._filesystems.keys()))
